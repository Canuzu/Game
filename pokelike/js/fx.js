...
(function (root) {
  ...
  function strahl(l, from, to, color, ms, dicke) {
    ...
    for (i = 0; i < n; i++) {
      ...
      var d = dicke || 10;
      var b = bit(l, from.x + dx * t, from.y + dy * t, d, d, color, 'fx-hart');
      var kern = bit(l, from.x + dx * t, from.y + dy * t, Math.max(4, d * 0.45),
        Math.max(4, d * 0.45), '#ffffff', 'fx-hart');
      animate(kern, [
  ...
  function wurf(l, from, to, color, ms, anzahl, hoehe, gestalt) {
    ...
    for (i = 0; i < count; i++) {
      var b = form(l, from.x, from.y, gestalt === 'blatt' ? 46 : 34,
        gestalt === 'blatt' ? 'blatt' : 'stoss', color, gestalt === 'blatt' ? 'fx-dreht' : '');
      // Auffächern statt aufreihen: Ohne Streuung quer zur Flugbahn wurde aus
      // elf Blättern ein grüner Strich.
      var streu = (i - count / 2) * 14;
      var quer = zufall(-18, 18);
      animate(b, [
        { transform: schiebe(0, 0, 0.7), opacity: 0 },
        { transform: schiebe((to.x - from.x) / 2 + quer, (to.y - from.y) / 2 - (hoehe || 30) + streu, 1), opacity: 1, offset: 0.5 },
        { transform: schiebe(to.x - from.x, to.y - from.y + streu * 0.35, 1), opacity: 1 }
      ], ms, i * 26);
    }
  }
  ...
  function steigen(l, at, color, anzahl, ms) {
    ...
    for (i = 0; i < anzahl; i++) {
      ...
      var b = form(l, x, at.boden, zufall(30, 52), 'stein', color);
      animate(b, [
        { transform: schiebe(0, 26, 0.4), opacity: 0 },
        { transform: schiebe(0, -zufall(30, 70), 1), opacity: 1, offset: 0.4 },
        { transform: schiebe(zufall(-18, 18), -zufall(50, 110), 0.8), opacity: 1, offset: 0.8 },
        { transform: schiebe(zufall(-24, 24), -zufall(60, 130), 0.6), opacity: 0 }
      ], ms, i * 30);
    }
  }
  ...
  function wirbel(l, at, color, ms, runden) {
    var i, n = 6;
    for (i = 0; i < n; i++) {
      var b = form(l, at.x, at.y, 36 + (i % 3) * 16, 'wirbelform', color);
  ...
  function funkeln(l, at, color, ms, anzahl) {
    ...
    for (i = 0; i < n; i++) {
      ...
      var b = form(l, x, y, zufall(26, 44), 'funke', i % 3 ? color : '#ffffff');
      animate(b, [
  ...
  function blasen(l, at, color, ms, anzahl) {
    ...
    for (i = 0; i < n; i++) {
      var x = at.x + zufall(-at.w * 0.45, at.w * 0.45);
      var b = form(l, x, at.boden - 20, zufall(26, 48), 'blase', i % 2 ? color : hell(color));
      animate(b, [
        { transform: schiebe(0, 20, 0.3), opacity: 0 },
        { transform: schiebe(zufall(-12, 12), -50, 1), opacity: 1, offset: 0.45 },
        { transform: schiebe(zufall(-20, 20), -120, 0.7), opacity: 0 }
      ], ms, i * 34);
    }
  }
  ...
  function aufblitzen(stage, color, ms, staerke) {
    if (reduced()) return;
    var f = doc.createElement('div');
    f.className = 'fx-blitzschirm';
    f.style.background = color;
    stage.appendChild(f);
    // Ein kurzes Zucken, kein Weißabgleich: Vorher lag der Schirm mit drei
    // Vierteln Deckkraft über der Bühne und wusch Pokémon wie Effekt aus.
    animate(f, [
      { opacity: 0 },
      { opacity: staerke || 0.3, offset: 0.15 },
      { opacity: 0 }
    ], ms || 200);
  }
  ...
  var TYP = {
    ...
    Grass: function (c) { wurf(c.l, c.from, c.to, c.farbe, 460, 9, 44, 'blatt'); return 520; },
    ...
    Fairy: function (c) { funkeln(c.l, c.to, c.farbe, 520, 16); welle(c.l, c.to, c.farbe, 460, 3); return 560; }
  };
  ...
  var ATTACKE = {
    ...
    thunder: function (c) { fallen(c.l, c.to, c.farbe, 5, 520, 'blitz'); aufblitzen(c.stage, '#fff6c0', 200, 0.45); shake(c.stage, 1); return 600; },
    thunderbolt: function (c) { fallen(c.l, c.to, c.farbe, 3, 440, 'blitz'); aufblitzen(c.stage, '#fff6c0', 180, 0.35); return 500; },
    ...
    hyperbeam: function (c) { strahl(c.l, c.from, c.to, '#e0a8ff', 620, 26); aufblitzen(c.stage, '#ffffff', 240, 0.35); shake(c.stage, 1); return 720; },
    ...
    razorleaf: function (c) { wurf(c.l, c.from, c.to, c.farbe, 480, 8, 52, 'blatt'); return 540; },
    ...
    explosion: function (c) { aufblitzen(c.stage, '#ffffff', 380, 0.55); platzen(c.l, c.from, '#ffd070', 24, 90, 520); shake(c.stage, 2); return 700; },
    ...
    psychic: function (c) { wirbel(c.l, c.to, c.farbe, 560, 2); aufblitzen(c.stage, '#f8d0f0', 220, 0.3); return 600; },
  ...
  function move(opts) {
    ...
    if (!opts.self) {
      ...
      root.setTimeout(function () {
        ...
        if (opts.eff > 1) aufblitzen(stage, '#ffffff', 180, 0.28);
